...
rule runSTARGenomeGenerate:
    ...
    shell:
        ...
        "{STAR_BINARY} --runThreadN 6 --runMode genomeGenerate --genomeDir {STAR_GENOME_DIR} --genomeFastaFiles {ANOTATION_FASTA_FILE}"
...
def determineBands(wildcards):
    bandCombination = wildcards.combBandNames
    pair = wildcards.pair
...
rule combineReads:
    input:
        determineBands
    output:
        SIM_DATA_BASE_PATH+"/sim_{simNum}/{bandProb}/{combBandNames}/Comb_R{pair}_sim.fq"
    shell:
        """
            mkdir -p {SIM_DATA_BASE_PATH}/sim_{wildcards.simNum}/{wildcards.bandProb}/{wildcards.combBandNames}
            cat {input} > {SIM_DATA_BASE_PATH}/sim_{wildcards.simNum}/{wildcards.bandProb}/{wildcards.combBandNames}/Comb_R{wildcards.pair}_sim.fq
        """
...
rule run_STAR_Band_INDIVIDUAL:
	...
	shell:
		...
		{STAR_BINARY} --runThreadN 32 --genomeDir {input.genomeDir} --readFilesIn {input.inputFilePair1} {input.inputFilePair2} --outSAMstrandField intronMotif --outFileNamePrefix {SIM_DATA_BASE_PATH}/sim_{wildcards.simNum}/{wildcards.bandProb}/{wildcards.band}/ --outSAMtype BAM SortedByCoordinate
		"""
...
rule run_STAR_Merged:
	...
	shell:
		...
		{STAR_BINARY} --runThreadN 32 --genomeDir {input.genomeDir} --readFilesIn {input.inputFilePair1} {input.inputFilePair2} --outSAMstrandField intronMotif --outFileNamePrefix {SIM_DATA_BASE_PATH}/sim_{wildcards.simNum}/{wildcards.bandProb}/merged/ --outSAMtype BAM SortedByCoordinate
		"""

rule run_STAR_Band_COMBINED:
        input:
                genomeDir = STAR_GENOME_DIR,
                inputFilePair1 = SIM_DATA_BASE_PATH+"/sim_{simNum}/{bandProb}/{combBandNames}/Comb_R1_sim.fq",
                inputFilePair2 = SIM_DATA_BASE_PATH+"/sim_{simNum}/{bandProb}/{combBandNames}/Comb_R2_sim.fq"
        threads:
                N_THREADS
        output:
                SIM_DATA_BASE_PATH+"/sim_{simNum}/{bandProb}/{combBandNames}/Comb.Aligned.sortedByCoord.out.bam"
        shell:
                """
                {STAR_BINARY} --runThreadN 32 --genomeDir {input.genomeDir} --readFilesIn {input.inputFilePair1} {input.inputFilePair2} --outSAMstrandField intronMotif --outFileNamePrefix {SIM_DATA_BASE_PATH}/sim_{wildcards.simNum}/{wildcards.bandProb}/{wildcards.combBandNames}/Comb. --outSAMtype BAM SortedByCoordinate
                """
