...
# uniqueMappingFile <- "/algbio1/shounak/Ladder_seq_final_repos/ladder_benchmark/simulations/sim_30000000/sim_1/betas_realistic/uniqueMappings.txt"
# percentageCutoff <- 0
# readCutoff <- 0
...
betaEstimator <- function(uniqueMappingFile, readCutoff, outputPath){
  ...
  ## writing the gene id transcript id file needed for the evaluations
  geneIdTransIDFilePath <- paste(outputPath,"geneId_transId.txt",sep = "")
  geneidTransIdFile <- tLength[,c(3,2)]
  names(geneidTransIdFile) <- c("ensembl_gene_id","ensembl_transcript_id")
  write.table(geneidTransIdFile,file=geneIdTransIDFilePath, sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)
  ...
  betaOutputFile <- paste(outputPath,"estimatedBeta_high_text.txt",sep = "")
